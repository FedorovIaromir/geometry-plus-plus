...
**Почти полная библиотека вычислительной геометрии на C++17**
...
**Geometry++** — это полноценная библиотека для вычислительной геометрии на C++17. Она предоставляет **более 120 функций** и **более 80 перегруженных операторов** для интуитивной работы с:
...
| `IntersectOut` | `set<Point> P; int num` | Результат пересечения (множество точек) |
...
### Расстояния
- `dist` — геометрическое
- `dist_O` — алгебраическое (ориентированное)
- `dist_S` — квадрат расстояния
